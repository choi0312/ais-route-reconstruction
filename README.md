...
본 저장소는 기존 대회 제출용 노트북을 그대로 업로드하지 않고, Feature Engineering, Kalman Filter + RTS Smoother, Temporal Delta Model, Graph Temporal Refinement, Block-wise Anchor Reconstruction, Submission 생성을 각각 모듈로 분리한 재현 가능한 Python 파이프라인으로 재구성한 버전입니다.

## Competition Result

| 항목 | 내용 |
|---|---|
| 대회명 | 스마트 해운물류 × AI 미션 챌린지 : 항로 복원 예측 |
| 플랫폼 | DACON |
...
- 평가 및 제출 로직을 별도 모듈로 분리해 재현성과 유지보수성 확보
